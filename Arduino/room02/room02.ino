...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("room02")) {
      Serial.println("connected");
      client.subscribe("myHome/lamp01/room02");
      client.subscribe("myHome/lightSensor/room02/threshold");
      client.subscribe("myHome/board/room02/debug");
    } else {
...
void readSensor() {
  ...
  if (event.light)
  {
    ...
    if (lux <= luxThreshold) {
      client.publish("myHome/lightSensor/room02", "OFF");
    }
    else {
      client.publish("myHome/lightSensor/room02", "ON");
    }
  }
  else
  {
  ...
  itoa(hum, strHum, 10);
  client.publish("myHome/temperatureSensor/room02", strTemp);
  client.publish("myHome/humiditySensor/room02", strHum);
  temp++;
  hum++;
}
...
void setLuxThreshold(char* _topic, char* _payload){
  if (0 == strcmp(_topic, "myHome/lightSensor/room02/threshold")){
...
void readBtns() {
  ...
  if (btn01 == true) {
    client.publish("myHome/windowSensor/room02", "OFF");
  }
  else {
    client.publish("myHome/windowSensor/room02", "ON");
  }
  if (btn02 == true) {
    client.publish("myHome/doorSensor/room02", "OFF");
  }
  else {
    client.publish("myHome/doorSensor/room02", "ON");
  }
}
...
void lampTopics(char* _topic, char* _payload){
  // Lamp 01
  if (0 == strcmp(_topic, "myHome/lamp01/room02")){
...
void debugTopic(char* _topic, char* _payload){
    // Lamp 01
  if (0 == strcmp(_topic, "myHome/board/room02/debug")){
    if (0 == strcmp(_payload, "1")){
      DEBUG = true;
    }
    else{
